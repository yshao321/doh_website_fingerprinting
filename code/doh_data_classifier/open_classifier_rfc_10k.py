...
def classifier_train():
    # Locate dataset
    closed_data_dir = join(abspath(join(dirname("__file__"), pardir, pardir)), 'dataset', 'closed-world')
    print(closed_data_dir)
    opened_data_dir = join(abspath(join(dirname("__file__"), pardir, pardir)), 'dataset', 'open-world')
    ...
    # Clean dataset
    df_closed = clean_df_closed(df_closed, min_packets, max_packets, num_classes, num_samples)
    print("cleaned closed data", df_closed.shape)
    df_opened = clean_df_opened(df_opened, min_packets, max_packets, 0, 1)
    print("cleaned opened data", df_opened.shape)

    # Generate a monitor list
    monitor_list = list(set(df_closed.class_label.tolist()))

    # Prepare cross-validation dataset
    df_cv = pd.concat([df_closed, df_opened])
